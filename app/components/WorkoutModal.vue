    ...
    <div class="relative glass max-w-2xl w-full rounded-3xl overflow-hidden animate-pop shadow-2xl">
      <div class="p-8">
        <div class="flex justify-between items-start mb-6">
          <div>
            <span class="text-blue-500 font-bold tracking-widest text-xs uppercase mb-1 block">{{ day?.day || 'Training' }}</span>
        ...
        <div class="space-y-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
          <!-- Main Workout Content -->
           <div v-if="day?.details">
               <ul class="space-y-3">
                   <li v-for="(exercise, idx) in day.details" :key="idx" class="flex items-start gap-3 p-3 rounded-xl bg-slate-900/30 border border-slate-700/30">
           ...
           <div v-else>
               <p class="text-slate-400">Geen details beschikbaar voor deze training.</p>
           </div>
      ...
      <div class="bg-slate-900/50 p-4 text-center border-t border-white/5">
        <button @click="$emit('close')" class="uppercase font-bold text-sm tracking-widest hover:text-blue-400 transition-colors">Sluiten</button>
      </div>
    </div>
  </div>
</template>
